...
public abstract class MultiblockControllerBase {
    ...
    private void onDetachBlock(IMultiblockPart part) {
        ...
        if (referenceCoord != null
                && referenceCoord.equals(part.getPos())) {
            referenceCoord = null;
        }
        ...
        if (!connectedParts.remove(part)) {
            BeefCoreLog
                    .warning(
                            ...
                            worldObj.isRemote ? "CLIENT" : "SERVER",
                            part.hashCode(), part.getPos().getX(), part.getPos().getY(),
                            part.getPos().getZ());
        }
    ...
    private String getPartsListString() {
        ...
        for (IMultiblockPart part : connectedParts) {
            ...
            sb.append(String.format("(%d: %d, %d, %d)", part.hashCode(),
                    part.getPos().getX(), part.getPos().getY(), part.getPos().getZ()));
            first = false;
        }
    ...
    private void auditParts() {
        ...
        for (IMultiblockPart part : connectedParts) {
            if (part.isInvalid()
                    || worldObj.getTileEntity(part.getPos()) != part) {
                onDetachBlock(part);
                deadParts.add(part);
            }
        }
    ...
    private void selectNewReferenceCoord() {
        ...
        for (IMultiblockPart part : connectedParts) {
            ...
            if (referenceCoord == null
                    || referenceCoord.compareTo(part.getPos().getX(), part.getPos().getY(),
                    part.getPos().getZ()) > 0) {
