...
public class RecipeCrafter
{
	 ...
	 */
	public RecipeCrafter(String recipeName, TileMachineBase parentTile, int inputs, int outputs, IInventory inventory,
	...
	public boolean canWork() {
		return this.parentTile.canWork() && this.currentRecipe != null;
	}
		...
		for (IBaseRecipeType recipe : RecipeHandler.getRecipeClassFromName(recipeName)) {
			if (recipe.canCraft(parentTile) && hasAllInputs(recipe)) {// This checks to see if it has all of the inputs
				...
				if (canGiveInvAll) {
					...
					//setIsActive();
					this.parentTile.markBlockForUpdate();
				}
				else {
					this.parentTile.machineResetProgress();
				}
			}
		}
	}
...
//				}
//			}
//			ArrayList<Integer> filledSlots = new ArrayList<>();// The slots that have been filled
...
//		}
	...
	private boolean canFitStack(ItemStack stack, int slot, boolean oreDic) {// Checks to see if it can fit the stack
		if (stack == null) {
			return true;
		}
	...
	private void fitStack(ItemStack stack, int slot) {// This fits a stack into a slot
		if (stack == null) {
			return;
		}
	...
	public void setCurrentRecipe(IBaseRecipeType recipe)
	{
		try
		{
			...
			this.parentTile.updateCostAndDuration(this.currentRecipe.euPerTick(), (int) (currentRecipe.tickTime() * (1.0 - speedMultiplier)));
