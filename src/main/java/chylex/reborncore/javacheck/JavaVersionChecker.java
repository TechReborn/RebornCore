...
public final class JavaVersionChecker
{
	...
	public static void run(JavaVersion minVersion)
	{
		...
		} catch (Throwable t)
		{
			t.printStackTrace();
			System.out.println(
					"Detected an unexpected error in Java Version Checker, ignoring since trying to run the game is more important.");
			System.out.println(
					"If you crashed and happen to see this, please report the error above to: " + issueReportSite);
		}
	}
	...
	private static void unsafeRun(JavaVersion minVersion)
	{
		...
		{
			if (minVersion == null)
				minVersion = JavaVersion.JAVA_1_8; // debugging purposes
	...
	public static class ShadingException extends RuntimeException
	{
		...
		{
			super("An exception happened when updating the coremod list, the mod you are shading Java Checker in will not run without it. Please, report the issue to "
					+ issueReportSite);
		}
	}
}
