...
public class BuiltContainer extends Container
{
	...
	public BuiltContainer(final String name, final EntityPlayer player, final List<IInventory> inventories, final Predicate<EntityPlayer> canInteract, final List<Range<Integer>> playerSlotRange, final List<Range<Integer>> tileSlotRange)
	{
		this.player = player;
		this.name = name;
		...
		if (!this.syncablesValues.isEmpty())
		{
			for (final SyncableProperty<?> syncable : this.syncablesValues)
			{
				if (syncable.needRefresh())
				{
					...
					new ContainerUpdatePacket(this.windowId, this.syncablesValues.indexOf(syncable), syncable.toNBT(new NBTTagCompound())).sendTo(this.player);
				}
			}
		}
	}
	...
	protected boolean shiftItemStack(final ItemStack stackToShift, final int start, final int end)
	{
		...
		if (stackToShift.isStackable())
		{
			for (int slotIndex = start; stackToShift.getCount() > 0 && slotIndex < end; slotIndex++)
			{
				...
				final ItemStack stackInSlot = slot.getStack();
				if (stackInSlot != ItemStack.EMPTY && ItemStack.areItemStacksEqual(stackInSlot, stackToShift) && slot.isItemValid(stackToShift))
				{
