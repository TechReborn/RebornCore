...
package reborncore.corelib.gui;
...
import reborncore.corelib.gui.slots.IPhantomSlot;
import reborncore.corelib.gui.slots.SlotBase;
import reborncore.corelib.inventory.StackHelper;
...
import reborncore.corelib.gui.widgets.Widget;
...
public abstract class BuildCraftContainer extends Container {
    ...
    public void sendWidgetDataToServer(Widget widget, byte[] data) {
        PacketGuiWidget pkt = new PacketGuiWidget(this.player, windowId, widgets.indexOf(widget), data);
        PacketHandler.sendPacketToServer(pkt);
    }

    public void handleWidgetData(int widgetId, byte[] data) {
        InputStream input = new ByteArrayInputStream(data);
        DataInputStream stream = new DataInputStream(input);
        if (widgetId < 0 || widgetId >= widgets.size()) RebornCore.logHelper.warn("Found a packet with an invalid widget ID! (" + widgetId + ")");
        else {
